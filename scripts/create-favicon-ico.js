...
function pngWidthHeight(pngBuffer) {
  // PNG IHDR chunk: width (4 bytes BE) at offset 16, height at 20
...
function buildIco(pngFiles, outFile) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    entries.writeUInt16LE(0, i * 16 + 4) // planes (0 for PNG)
    entries.writeUInt16LE(0, i * 16 + 6) // bitcount (0 for PNG)
